...
export const PRAYER_STATUSES = Object.freeze(['on_time', 'late', 'missed']);
...
export function getPrayerTracker(date = new Date()) {
  ...
  return TRACKED_PRAYERS.reduce((result, prayer) => {
    ...
    // Preserve tracker entries created by earlier yes/no versions.
    result[prayer] = value === true ? 'on_time' : PRAYER_STATUSES.includes(value) ? value : null;
    return result;
  }, {});
}

export function setPrayerStatus(prayer, status = null, date = new Date()) {
  ...
  if (PRAYER_STATUSES.includes(status)) current[prayer] = status;
  else delete current[prayer];
  records[key] = current;
...
export function togglePrayerTracked(prayer, date = new Date()) {
  const current = getPrayerTracker(date);
  return setPrayerStatus(prayer, current[prayer] ? null : 'on_time', date);
}
