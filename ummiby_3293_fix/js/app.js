...
import { renderShell, saveActiveJourney } from "./shell.js?v=3.39.0";
...
import {
  ...
  signOut,
  signUpWithPassword
} from "./auth.js?v=3.29.0";
import { initializeSupabase, getSupabaseClient } from "./supabase.js?v=3.29.0";
...
import { getPrayerSettings, savePrayerSettings, requestAutomaticLocation, calculatePrayerTimes, formatPrayerTime, getNextPrayer, getPrayerTracker, togglePrayerTracked, setPrayerStatus } from './services/prayer-times.js?v=3.39.0';
...
function bindShellEvents() {
  ...
  app.querySelectorAll('[data-featured-history-month]').forEach(button=>button.addEventListener('click',()=>{const date=new Date(quranHistoryView.year,quranHistoryView.month+Number(button.dataset.featuredHistoryMonth),1);quranHistoryView={year:date.getFullYear(),month:date.getMonth()};render()}));
  const featuredPage=app.querySelector('[data-featured-history-kind]'),featuredDialog=app.querySelector('[data-featured-day-dialog]');let selectedFeaturedDay='';
  app.querySelectorAll('[data-edit-featured-day]').forEach(button=>button.addEventListener('click',()=>{selectedFeaturedDay=button.dataset.editFeaturedDay;const kind=featuredPage?.dataset.featuredHistoryKind,keys={kahf:'ummiby.quran.kahfFriday.records',mulk:'ummiby.quran.mulkFeatured.records',sajdah:'ummiby.quran.sajdahFeatured.records'},key=keys[kind];let records={};try{records=JSON.parse(localStorage.getItem(key)||'{}')}catch{}const title=featuredDialog?.querySelector('[data-featured-day-title]');if(title)title.textContent=new Date(`${selectedFeaturedDay}T12:00:00`).toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});const record=records[selectedFeaturedDay],status=record?.status==='partial'?'partial':record&&(record.status==='complete'||record.completed||record.source)?'complete':'none';featuredDialog?.querySelectorAll('[data-featured-day-status]').forEach(input=>{input.checked=input.value===status});featuredDialog?.showModal()}));
  app.querySelectorAll('[data-close-featured-day]').forEach(button=>button.addEventListener('click',()=>featuredDialog?.close()));
  app.querySelector('[data-save-featured-day]')?.addEventListener('click',()=>{if(!selectedFeaturedDay)return;const kind=featuredPage?.dataset.featuredHistoryKind,keys={kahf:'ummiby.quran.kahfFriday.records',mulk:'ummiby.quran.mulkFeatured.records',sajdah:'ummiby.quran.sajdahFeatured.records'},key=keys[kind];let records={};try{records=JSON.parse(localStorage.getItem(key)||'{}')}catch{}const status=featuredDialog.querySelector('[data-featured-day-status]:checked')?.value||'none';if(status==='none')delete records[selectedFeaturedDay];else records[selectedFeaturedDay]={...(records[selectedFeaturedDay]||{}),status,source:'manual',readingMethod:'external',updatedAt:new Date().toISOString()};localStorage.setItem(key,JSON.stringify(records));if(status!=='none'){let readingDays={};try{readingDays=JSON.parse(localStorage.getItem('ummiby.quran.readingDays')||'{}')}catch{}readingDays[selectedFeaturedDay]={source:`${kind}-${status==='complete'?'full':'partial'}`,updatedAt:new Date().toISOString()};localStorage.setItem('ummiby.quran.readingDays',JSON.stringify(readingDays))}featuredDialog.close();toast(status==='complete'?'Full reading recorded.':status==='partial'?'Partial reading recorded.':'Reading status set to not read.');render()});
  app.querySelector('[data-reset-featured-history]')?.addEventListener('click',event=>{const button=event.currentTarget,kind=button.dataset.resetFeaturedHistory,keys={kahf:'ummiby.quran.kahfFriday.records',mulk:'ummiby.quran.mulkFeatured.records',sajdah:'ummiby.quran.sajdahFeatured.records'},key=keys[kind];if(!key)return;if(!confirm('Clear this Surah’s entire reading history? Current section progress will not be changed.'))return;localStorage.setItem(key,'{}');let readingDays={};try{readingDays=JSON.parse(localStorage.getItem('ummiby.quran.readingDays')||'{}')}catch{}Object.keys(readingDays).forEach(day=>{if(String(readingDays[day]?.source||'').startsWith(`${kind}-`))delete readingDays[day]});localStorage.setItem('ummiby.quran.readingDays',JSON.stringify(readingDays));toast('Reading history was reset.');render()});
  ...
  app.querySelector('[data-kahf-reset-friday]')?.addEventListener('click',()=>{saveKahf(active=>{active.completedSections=[];active.currentSection=1;active.savedAyah=0;delete active.completedAt;active.lastActivityAt=new Date().toISOString()});toast('This Friday’s Al-Kahf section progress was reset. Calendar history was preserved.');render()});
  ...
  const saveMulk=(mutator)=>{const cycleDate=localDateKey(new Date());let active={};try{active=JSON.parse(localStorage.getItem('ummiby.quran.mulkFeatured.active')||'{}')}catch{}active=active.cycleDate===cycleDate?{completedSections:[],currentSection:1,savedAyah:0,...active}:{cycleDate,completedSections:[],currentSection:1,savedAyah:0};mutator(active);localStorage.setItem('ummiby.quran.mulkFeatured.active',JSON.stringify(active));return active};
  const syncMulkRecord=(active)=>{let records={};try{records=JSON.parse(localStorage.getItem('ummiby.quran.mulkFeatured.records')||'{}')}catch{}const key=localDateKey(new Date()),count=(active.completedSections||[]).length;if(!count)delete records[key];else{const existing=records[key]||{},status=count===4?'complete':'partial';records[key]={...existing,status,source:'app',updatedAt:new Date().toISOString()};recordQuranReading(status==='complete'?'mulk-full':'mulk-partial')}localStorage.setItem('ummiby.quran.mulkFeatured.records',JSON.stringify(records))};
  ...
  const saveSajdah=(mutator)=>{const cycleDate=localDateKey(new Date());let active={};try{active=JSON.parse(localStorage.getItem('ummiby.quran.sajdahFeatured.active')||'{}')}catch{}active=active.cycleDate===cycleDate?{completedSections:[],currentSection:1,savedAyah:0,...active}:{cycleDate,completedSections:[],currentSection:1,savedAyah:0};mutator(active);localStorage.setItem('ummiby.quran.sajdahFeatured.active',JSON.stringify(active));return active};
  const syncSajdahRecord=(active)=>{let records={};try{records=JSON.parse(localStorage.getItem('ummiby.quran.sajdahFeatured.records')||'{}')}catch{}const key=localDateKey(new Date()),count=(active.completedSections||[]).length;if(!count)delete records[key];else{const existing=records[key]||{},status=count===4?'complete':'partial';records[key]={...existing,status,source:'app',updatedAt:new Date().toISOString()};recordQuranReading(status==='complete'?'sajdah-full':'sajdah-partial')}localStorage.setItem('ummiby.quran.sajdahFeatured.records',JSON.stringify(records))};
  app.querySelectorAll('[data-sajdah-return-index]').forEach(link=>link.addEventListener('click',()=>{const section=Number(link.dataset.sajdahReturnIndex);if(section)saveSajdah(active=>{active.currentSection=section;active.lastActivityAt=new Date().toISOString()})}));
  app.querySelectorAll('[data-sajdah-month]').forEach(button=>button.addEventListener('click',()=>{const current=Number(localStorage.getItem('ummiby.quran.sajdahFeatured.calendarOffset')||0);localStorage.setItem('ummiby.quran.sajdahFeatured.calendarOffset',String(current+Number(button.dataset.sajdahMonth)));render()}));
  app.querySelectorAll('[data-sajdah-save-ayah]').forEach(button=>button.addEventListener('click',()=>{const ayah=Number(button.dataset.sajdahSaveAyah),section=Number(app.querySelector('[data-sajdah-section]')?.dataset.sajdahSection||1);saveSajdah(active=>{active.currentSection=section;active.savedAyah=ayah;active.lastActivityAt=new Date().toISOString()});toast(`Saved at Surah 32 • As-Sajdah, Ayah ${ayah}.`);render()}));
  app.querySelector('[data-sajdah-complete-section]')?.addEventListener('click',button=>{const section=Number(button.dataset.sajdahCompleteSection);let removed=false;const active=saveSajdah(active=>{const completed=new Set(active.completedSections||[]);if(completed.has(section)){completed.delete(section);removed=true;active.currentSection=section}else{completed.add(section);active.currentSection=Math.min(4,section+1);active.savedAyah=0}active.completedSections=[...completed].sort((a,b)=>a-b);active.lastActivityAt=new Date().toISOString()});syncSajdahRecord(active);toast(removed?'Section marked unread.':active.completedSections.length===4?'Surah As-Sajdah completed.':'Section marked complete.');render()});
  app.querySelector('[data-sajdah-complete-surah]')?.addEventListener('click',()=>{saveSajdah(active=>{active.completedSections=[1,2,3,4];active.currentSection=4;active.savedAyah=0;active.completedAt=new Date().toISOString()});let records={};try{records=JSON.parse(localStorage.getItem('ummiby.quran.sajdahFeatured.records')||'{}')}catch{}records[localDateKey(new Date())]={status:'complete',source:'app',updatedAt:new Date().toISOString()};localStorage.setItem('ummiby.quran.sajdahFeatured.records',JSON.stringify(records));recordQuranReading('sajdah-full');toast('Surah As-Sajdah completed.');navigate('/quran/as-sajdah')});


  app.querySelector('[data-mulk-reset-progress]')?.addEventListener('click',()=>{saveMulk(active=>{active.completedSections=[];active.currentSection=1;active.savedAyah=0;delete active.completedAt});toast('Tonight’s Al-Mulk progress was reset.');render()});
  app.querySelector('[data-sajdah-reset-progress]')?.addEventListener('click',()=>{saveSajdah(active=>{active.completedSections=[];active.currentSection=1;active.savedAyah=0;delete active.completedAt});toast('Tonight’s As-Sajdah progress was reset.');render()});

  const featuredLanding=app.querySelector('[data-featured-landing-kind]'),featuredLandingDialog=app.querySelector('[data-featured-landing-dialog]');let featuredLandingDate='';
  app.querySelectorAll('[data-edit-featured-landing-day]').forEach(button=>button.addEventListener('click',()=>{featuredLandingDate=button.dataset.editFeaturedLandingDay;const title=featuredLandingDialog?.querySelector('[data-featured-landing-date]');if(title)title.textContent=new Date(`${featuredLandingDate}T12:00:00`).toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric',year:'numeric'});if(featuredLandingDialog){const rect=button.getBoundingClientRect();featuredLandingDialog.style.setProperty('--popover-left',`${Math.min(window.innerWidth-340,Math.max(12,rect.left-110))}px`);featuredLandingDialog.style.setProperty('--popover-top',`${Math.min(window.innerHeight-310,rect.bottom+8)}px`);featuredLandingDialog.show()}}));
  app.querySelectorAll('[data-close-featured-landing-dialog]').forEach(button=>button.addEventListener('click',()=>featuredLandingDialog?.close()));
  featuredLandingDialog?.addEventListener('click',event=>{if(event.target===featuredLandingDialog)featuredLandingDialog.close()});
  app.querySelectorAll('[data-set-featured-landing-status]').forEach(button=>button.addEventListener('click',()=>{if(!featuredLandingDate||!featuredLanding)return;const kind=featuredLanding.dataset.featuredLandingKind,keys={kahf:'ummiby.quran.kahfFriday.records',mulk:'ummiby.quran.mulkFeatured.records',sajdah:'ummiby.quran.sajdahFeatured.records'},key=keys[kind];let records={};try{records=JSON.parse(localStorage.getItem(key)||'{}')}catch{}const status=button.dataset.setFeaturedLandingStatus;if(status==='not-read')delete records[featuredLandingDate];else records[featuredLandingDate]={...(records[featuredLandingDate]||{}),status,source:'manual',readingMethod:'external',updatedAt:new Date().toISOString()};localStorage.setItem(key,JSON.stringify(records));featuredLandingDialog.close();toast(status==='complete'?'Marked fully read.':status==='partial'?'Marked partially read.':'Marked not read.');render()}));
